...
let DECIMAL_1E9 = BigDecimal.fromString('1e9')
let DECIMAL_1E12 = BigDecimal.fromString('1e12')
...
export function getETHUSDRate(): BigDecimal {
    ...
    let ethRate = reserve0.div(reserve1).times(DECIMAL_1E12)
...
export function getOHMUSDRate(): BigDecimal {
    ...
    let reserve0 = reserves.value0.toBigDecimal()
    let reserve1 = reserves.value1.toBigDecimal()
    let ohmRate = reserve1.div(reserve0).div(DECIMAL_1E9)
...
export function getXSushiUSDRate(): BigDecimal {
    ...
    let reserves = pair.getReserves()
    let reserve0 = reserves.value0.toBigDecimal()
    let reserves1 = reserves.value1.toBigDecimal()
    let xsushiRate = reserve1.div(reserve0).times(getETHUSDRate())
    log.debug("xsushi Rate {}", [xsushiRate.toString()])
    return xsushiRate    
}
...
export function getCVXUSDRate(): BigDecimal {
    ...
    let reserves = pair.getReserves()
    let reserve0 = reserves.value0.toBigDecimal()
    let reserve1 = reserve0.value1.toBigDecimal()
    let cvxRate = reserve1.div(reserve0).times(getETHUSDRate())
...
export function getPairWETH(lp_amount: BigInt, pair_address: string): BigDecimal {
    let pair = UniswapV2Pair.bind(Address.fromString(pair_address))
    let total_lp = pair.totalSupply()
    let lp_token_0 = pair.getReserves().value0
    let lp_token_1 = pair.getReserves().value1
